...
import dev.weixiao.wxfilemanager.databinding.DialogSmbConnectBinding
import dev.weixiao.wxfilemanager.databinding.DialogViewSettingsBinding
...
class SmbFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.swipeRefresh.setOnRefreshListener {
            when (viewModel.viewState.value) {
                ...
                SmbViewModel.ViewState.FILES -> {
                    SmbManager.clearCacheForPath(viewModel.currentPath.value ?: "")
                    viewModel.loadFiles(viewModel.currentPath.value ?: "", useCache = false)
                }
    ...
    fun showSearchDialog() {
        val intent = android.content.Intent(requireContext(), dev.weixiao.wxfilemanager.ui.viewer.SearchActivity::class.java).apply {
    ...
    private fun showServers(servers: List<dev.weixiao.wxfilemanager.model.SmbServer>) {
    ...
    private fun updateBreadcrumbUI(crumbs: List<SmbViewModel.BreadcrumbItem>) {
        ...
        for (i in crumbs.indices) {
            val crumb = crumbs[i]
            val crumbBinding = dev.weixiao.wxfilemanager.databinding.ItemBreadcrumbBinding.inflate(layoutInflater, binding.breadcrumbContainer, false)
    ...
    private fun handleFileClick(file: FileModel) {
        if (file.isImage) {
            ...
            dev.weixiao.wxfilemanager.utils.MediaRepository.setImageList(imageList)
            
            val intent = android.content.Intent(requireContext(), dev.weixiao.wxfilemanager.ui.viewer.ImageViewerActivity::class.java).apply {
                putExtra("position", position)
            }
            startActivity(intent)
        } else if (file.isVideo) {
            val intent = android.content.Intent(requireContext(), dev.weixiao.wxfilemanager.ui.viewer.VlcVideoPlayerActivity::class.java).apply {
